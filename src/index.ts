...
app.whenReady().then(() => {
    ...
    callback({
      responseHeaders: {
        ...
        'Content-Security-Policy': [
          'default-src \'self\' \'unsafe-inline\' \'unsafe-eval\' https://*.mock.pstmn.io data: blob: file:',
        ],
      },
    });
  });
  ...
  ipcMain.on('load-chat-session', (event, sessionId) => {
    ...
    const chatSession = newChat.readFromFile(sessionId);
    mainWindow.webContents.send('chat-session-load-reply', chatSession);
  });

  ipcMain.on('write-chat-session', (event, chatSession: ChatSession) => {
    console.log('Write chat session requested.', chatSession);
    newChat.writeToFile(chatSession);
  });
