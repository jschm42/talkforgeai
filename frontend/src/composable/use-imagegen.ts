...
export class ImageGenRequest {
  imagePrompt = '';
}

export function useImageGen() {
  const textToImage = async (request: ImageGenRequest): Promise<any> => {
    const result = await axios.post('/api/v1/imagegen/text2image', request);
    return result.data;
  };
